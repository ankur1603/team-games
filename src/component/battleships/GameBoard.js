...
const GameBoard = (props) => {
  ...
  return (
    <>
      <div>
        {(gameStarted == true) ? <GameLegend/> : <ShipLegend updateColorSelection={setSelectedShipColor}/>}
          ...
          {matrix.map((row, rowIndex) =>
            row.map((cell, colIndex) => (
              <div
                ...
                onDoubleClick={(event) => handleDoubleClick(rowIndex, colIndex, event)}
                style={{
                  width: 40,
                  height: 40,
              ...
              >
                {cell['hit_miss']=='hit' ? "❌" : cell['hit_miss']=='miss'? "🔘": cell['hit_miss']=='destroyed'? "🔥": `${rowIndex}${colIndex}`}
              </div>
            ))
          )}
        </div>
